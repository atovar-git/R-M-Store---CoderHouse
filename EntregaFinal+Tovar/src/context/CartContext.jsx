...
const CartProvider = (props) => {                                      // Este provider "es como un componente padre" el cual provee un ESTADO y una FUNCION para que los componentes puedan modificarlo.
    ...
    const addProductCart = (product, amount, cb = null) => {
        const aux = cart;                                              // Guardamos el carrito en un variable temporal.

        const index = aux.findIndex(item => item.id == product.id)     // Si YA existe o no. findIndex permite pasar un callback particular.

        if (index != -1) {                                             // Si YA existe, cambio la cantidad.
            aux[index].amount = amount
        } else {                                                       // Si NO existe lo agrego.
            const newCartItem = { ...product, amount }                 // Solo inserto un objeto (amount solo es igual a amount: amount ES6).
    ...
    return (
        <>
            {/* Ofrece el estado y las funciones para modificarlo. Los componentes usarán dicha función. */}
            <CartContext.Provider value={{ cart, addProductCart, deleteProductCart, cleanCart, amountInCart }}>
                {props.children}
